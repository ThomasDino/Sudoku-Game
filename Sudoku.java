...
public class Sudoku {
	...
	public boolean enterNumber(int i, int j, int num) {
	    // Check if the location is updatable
	    if (!isLocationUpdatable(i, j)) {
	        return false;
	    }

	    // Check if i, j, and num are within valid ranges
	    if (i < 0 || i >= N || j < 0 || j >= N || num < 1 || num > 9) {
	        return false;
	    }

	    // Check if the spot to be updated is initialized with zero
	    if (mat[i][j] != 0) {
	        return false;
	    }

	    // Check if num violates Sudoku rules in the row
	    ...
	    // Check if num violates Sudoku rules in the column
	    ...
	    // Check if num violates Sudoku rules in the 3x3 sub-grid
	    int startRow = (i / 3) * 3;
	    int startCol = (j / 3) * 3;
	    for (int row = 0; row < 3; row++) {
	        for (int col = 0; col < 3; col++) {
	            if (mat[startRow + row][startCol + col] == num) {
	                return false;
	            }
	        }
	    }

	    // Update the spot on the board
	    mat[i][j] = num;
	    missingLoc.remove(i + "-" + j); // Remove from missing locations
	    R--; // Decrement remaining missing digits
